	...
	if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
	{
		...
		for (unsigned i = 0; i < vr::k_EButton_Max; ++i)
		{
			...
			if (!oldTouchedState && newTouchedState)
			{
				...
				vr::VRServerDriverHost_004()->TrackedDeviceButtonTouched(m_openvrId, (vr::EVRButtonId)i, offset);
			}
			else if (oldTouchedState && !newTouchedState)
			{
				LOG(DEBUG) << m_serialNumber << ": ButtonUntouch detected: " << i;
				vr::VRServerDriverHost_004()->TrackedDeviceButtonUntouched(m_openvrId, (vr::EVRButtonId)i, offset);
			}
			...
			if (!oldPressedState && newPressedState)
			{
				...
				vr::VRServerDriverHost_004()->TrackedDeviceButtonPressed(m_openvrId, (vr::EVRButtonId)i, offset);
			}
			else if (oldPressedState && !newPressedState)
			{
				LOG(DEBUG) << m_serialNumber << ": ButtonUnpress detected: " << i;
				vr::VRServerDriverHost_004()->TrackedDeviceButtonUnpressed(m_openvrId, (vr::EVRButtonId)i, offset);
			}
		}
		...
		for (unsigned i = 0; i < vr::k_unControllerStateAxisCount; ++i)
		{
			if (oldState.rAxis[i].x != newState.rAxis[i].x || oldState.rAxis[i].y != newState.rAxis[i].y)
			{
				...
				vr::VRServerDriverHost_004()->TrackedDeviceAxisUpdated(m_openvrId, i, newState.rAxis[i]);
			}
		}
	}
	else
...
void VirtualDeviceDriver::buttonEvent(ButtonEventType eventType, uint32_t buttonId, double timeOffset, bool notify)
{
	...
	case ButtonEventType::ButtonPressed:
		...
		if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
		{
			vr::VRServerDriverHost_004()->TrackedDeviceButtonPressed(m_openvrId, (vr::EVRButtonId)buttonId, timeOffset);
		}
		break;
	case ButtonEventType::ButtonUnpressed:
		...
		if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
		{
			vr::VRServerDriverHost_004()->TrackedDeviceButtonUnpressed(m_openvrId, (vr::EVRButtonId)buttonId, timeOffset);
		}
		break;
	case ButtonEventType::ButtonTouched:
		...
		if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
		{
			vr::VRServerDriverHost_004()->TrackedDeviceButtonTouched(m_openvrId, (vr::EVRButtonId)buttonId, timeOffset);
		}
		break;
	case ButtonEventType::ButtonUntouched:
		...
		if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
		{
			vr::VRServerDriverHost_004()->TrackedDeviceButtonUntouched(m_openvrId, (vr::EVRButtonId)buttonId, timeOffset);
		}
		break;
	default:
		break;
...
void VirtualDeviceDriver::axisEvent(uint32_t axisId, const vr::VRControllerAxis_t &axisState, bool notify)
{
	...
	if (axisId < vr::k_unControllerStateAxisCount)
	{
		...
		if (notify && m_openvrId != vr::k_unTrackedDeviceIndexInvalid)
		{
			vr::VRServerDriverHost_004()->TrackedDeviceAxisUpdated(m_openvrId, axisId, m_ControllerState.rAxis[axisId]);
		}
	}
}
